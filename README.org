...
AUCTeX provides commands for generating previews in various regions: the current section, the entire document, the marked region, and so on.  While these previews generate, you're not supposed to edit them, because that can mess up their positioning.  A typical workflow is thus to run the command =preview-section= (=C-c C-p C-s=) every few minutes, during pauses in editing.  This introduces a bit of overhead if you prefer to have previews on by default.
...
In recent auctex versions the `preview-leave-open-previews-visible` option has been replaced by two new options:
#+begin_src elisp
...
When you visit a revision or version backup created by =vc.el= (e.g., via =C-x v ~=), Emacs uses file names like =paper.tex.~REV~=.  AUCTeX decides whether a buffer's file already has a TeX extension using =TeX-file-extensions=, and =preview-latex= (hence =preview-auto=) relies on that when computing the master file via =TeX-master-file=.  The default list does not match =tex.~REV~=, so AUCTeX treats these files as extension-less and appends =.tex=, producing wrong master/aux names and breaking previews.  Add the following to recognize VC revision files as TeX sources:
#+begin_src elisp
